...
- 예시:
  ...
  • V1**init.sql: 데이터베이스 초기화와 테이블 생성
  • V2**add_new_column.sql: 새로운 컬럼 추가
  • V3**modify_table.sql: 기존 테이블 수정
  • V4**add_index.sql: 인덱스 추가

- 파일 이름 규칙:

  • 파일명은 반드시 V + 버전번호로 시작해야 하며, 그 뒤에 마이그레이션의 내용에 대한 설명을 추가하는 것이 일반적입니다.
  • 버전 번호는 정수로 구성되며, 점(.)을 포함할 수 있습니다(예: V1.1\_\_...).

- Flyway의 동작 방식:

  1.  Flyway는 flyway_schema_history 테이블에 기록된 마이그레이션을 확인합니다.
  2.  아직 실행되지 않은 새로운 마이그레이션 파일(예: V3\_\_...)이 있으면 이를 실행합니다.
  3.  마이그레이션이 성공적으로 완료되면 해당 파일의 정보가 **flyway_schema_history**에 기록됩니다.

- 새 마이그레이션 파일 추가할 때의 예시:

  1.  새로운 테이블을 추가하고 싶다면 V3\_\_add_new_table.sql을 생성하여 해당 SQL을 작성한 뒤 Flyway로 마이그레이션을 실행합니다.
  2.  기존 테이블에 수정 사항이 생기면 V4\_\_update_existing_table.sql을 생성하고 수정 SQL을 작성합니다.
